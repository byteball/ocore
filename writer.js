...
function saveJoint(objJoint, objValidationState, preCommitCallback, onDone) {
	...
	db.takeConnectionFromPool(conn => {
		...
		if (bGenesis)
			...
			conn.addQuery(arrQueries, "UPDATE units SET is_free=0 WHERE unit IN(?)", [objUnit.parent_units], ({affectedRows}) => {
				// in sqlite3, result.affectedRows actually returns the number of _matched_ rows
				const count_consumed_free_units = affectedRows;
				...
				if (message.payload_location === "inline"){
					switch (message.app){
						...
						case "vote":
							...
							break;
						case "attestation":
							const attestation = message.payload;
						...
						case "asset_attestors":
							const asset_attestors = message.payload;
		...
		function determineInputAddressFromSrcOutput(asset, denomination, {unit, message_index, output_index}, handleAddress) {
			conn.query(
				...
				(message, i, cb2) => {
					...
					async.forEachOfSeries(
						...
						(input, j, cb3) => {
							...
							const determineInputAddress = handleAddress => {
							...
							determineInputAddress(address => {
								const is_unique = 
									objValidationState.arrDoubleSpendInputs.some(({message_index, input_index}) => message_index === i && input_index === j) 
									? null : 1;
		...
		function updateBestParent(cb){
			...
			conn.query(
				...
				rows => {
					...
					conn.query("UPDATE units SET best_parent_unit=? WHERE unit=?", [my_best_parent_unit, objUnit.unit], () => { cb(); });
				}
			);
		}
		...
		function determineMaxLevel(handleMaxLevel){
			...
			async.each(
				...
				(parent_unit, cb) => {
					storage.readStaticUnitProps(conn, parent_unit, ({level}) => {
						if (level > max_level)
							max_level = level;
						cb();
					});
...
function readCountOfAnalyzedUnits(handleCount){
	...
	db.query("SELECT * FROM sqlite_master WHERE type='table' AND name='sqlite_stat1'", ({length}) => {
		if (length === 0)
