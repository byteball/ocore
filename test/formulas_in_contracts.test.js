...
test('formula - input_x0 - error', t => {
	evalFormulaBB.validate("input_x0.amount * outputs_x1.amount == 10", 2,
		(result) => {
			t.not(result.error.match(/parse error/), null);
			t.deepEqual(result.complexity, 2);
		});
...
test('formula - correct operator in address in output - ok - 2', t => {
	evalFormulaBB.validate("output[address!=MXMEKGN37H5QO2AWHT7XRG6LHJVVTAWU].amount == 20", 1, (result) => {
		t.is(result.error, false);
		t.deepEqual(result.complexity, 1);
	});
});
...
test('formula - correct operator in asset in output - ok - 1', t => {
	evalFormulaBB.validate("output[asset='p+U9OB+JOCW5/7hXiRpVw65HwzFprNfj68PCy/7BR6A='].amount == 20", 1, (result) => {
		t.is(result.error, false);
		t.deepEqual(result.complexity, 1);
	});
});
...
test('formula - != operator in asset in output - ok', t => {
	evalFormulaBB.validate("output[asset!='p+U9OB+JOCW5/7hXiRpVw65HwzFprNfj68PCy/7BR6A='].amount == 20", 5, (result) => {
		t.is(result.error, false);
...
test('formula - other address in input - ok', t => {
	evalFormulaBB.validate("input[address=other address].amount == 20", 3, (result) => {
		t.is(result.error, false);
...
test('formula - this address in output - error', t => {
	evalFormulaBB.validate("output[address=this address] == 20", 3, (result) => {
		t.not(result.error.match(/parse error/), null);
		t.deepEqual(result.complexity, 3);
	});
});
...
test.cb('formula - ifseveral=last', t => {
	evalFormulaBB.validate("data_feed[oracles=\"MXMEKGN37H5QO2AWHT7XRG6LHJVVTAWU\", feed_name=\"test\", ifseveral=\"last\"] == 10", 3,
		(result) => {
			t.is(result.error, false);
