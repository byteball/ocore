...
function validateDefinition(
    ...
    arrDefinition,
    {authors},
    {bDefiningPrivateAsset, last_ball_mci, bNoReferences},
    arrAuthentifierPaths,
    bAssetCondition,
    handleResult
) {
	...
	function getFilterError(filter){
		...
		if (bAssetCondition && filter.asset === "this asset" && bDefiningPrivateAsset)
	...
	function determineIfAnyOfAssetsIsPrivate(arrAssets, cb){
		if (arrAssets.length === 0)
			return cb(false);
		conn.query("SELECT 1 FROM assets WHERE unit IN(?) AND is_private=1 LIMIT 1", [arrAssets], ({length}) => {
			cb(length > 0);
		});
	}
	...
	function needToEvaluateNestedAddress(path){
		if (!arrAuthentifierPaths) // no signatures, just validating a new definition
			return true;
		if (last_ball_mci < 1400000) // skipping is enabled after this mci
			return true;
	...
	function evaluate(arr, path, bInNegation, cb){
		...
		switch(op){
			...
			case 'weighted and':
				...
				if (args.set.length < 2)
					return cb("set must have at least 2 options");
				let weight_of_options_with_sig = 0;
				let total_weight = 0;
				var index = -1;
				async.eachSeries(
					args.set,
			...
			case 'address':
				...
				if (bAssetCondition)
					return cb(`asset condition cannot have ${op}`);
				const other_address = args;
				if (!isValidAddress(other_address))
					return cb("invalid address");
				storage.readDefinitionByAddress(conn, other_address, last_ball_mci, {
					...
					//		return cb(null, true);
						...
						const arrDefiningAuthors = authors.filter(({address, definition}) => address === other_address && definition && objectHash.getChash160(definition) === definition_chash);
			...
			case 'has definition change':
				...
				const changed_address = args[0];
				const new_definition_chash = args[1];
	...
	function evaluate(arr, path, cb2){
		...
		switch(op){
			...
			case 'sig':
				...
				//console.log(op, path);
				const signature = assocAuthentifiers[path];
				if (!signature)
					return cb2(false);
			...
			case 'seen':
				...
				const filter = args;
				let sql = `SELECT 1 FROM ${filter.what}s CROSS JOIN units USING(unit) \n\
			...
			case 'in data feed':
				...
				conn.query(
					...
					params,
					({length}) => {
						console.log(`${op} ${feed_name} ${length}`);
						cb2(length > 0);
					}
				);
				break;
			...
			case 'in merkle':
				...
				const serialized_proof = assocAuthentifiers[path];
				const proof = merkle.deserializeMerkleProof(serialized_proof);
			...
			case 'age':
				...
				conn.query(
					...
					({length}) => {
						const bSatisfies = (length === arrSrcUnits.length);
			...
			case 'has one equal':
				...
				augmentMessagesAndEvaluateFilter("has", args.search_criteria[0], (res1, arrFirstObjects) => {
					...
					augmentMessagesAndEvaluateFilter("has", args.search_criteria[1], (res2, arrSecondObjects) => {
						...
						for (let i=0; i<arrFirstObjects.length; i++)
							for (let j=0; j<arrSecondObjects.length; j++)
								if (!args.equal_fields.some(field => arrFirstObjects[i][field] !== arrSecondObjects[j][field]))
									count_equal_pairs++;
			...
			case 'has definition change':
				...
				if (new_definition_chash === 'this address')
					new_definition_chash = address;
				cb2(objUnit.messages.some(({app, payload}) => {
					if (app !== 'address_definition_change')
						return false;
					if (!payload)
						return false;
					if (payload.definition_chash !== new_definition_chash)
						return false;
					const address = payload.address || objUnit.authors[0].address;
