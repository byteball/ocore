...
exports.verifyWithPemKey = function(hash, signature, pem_key) {
...
exports.validateAndFormatPemKey = function(pem_key, handle) {
		...
		//we remove any return carriage before treatment
		pem_key = pem_key.replace(/[\n\r]/g, '').trim();
		
		//we add header and footer if not present
		if (pem_key.indexOf("-----BEGIN PUBLIC KEY-----") !== 0)
			pem_key = "-----BEGIN PUBLIC KEY-----" + pem_key;
		if (pem_key.indexOf("-----END PUBLIC KEY-----") !== (pem_key.length - 24))
			pem_key = pem_key + "-----END PUBLIC KEY-----";
	
		var encoding_alone_b64 = pem_key.slice(26,(pem_key.length - 24));
		if (!ValidationUtils.isValidBase64(encoding_alone_b64))
			return handle("base64 encoding is not valid");
		...
		if (!objSupportedPemCurves[curveIdentifiersHex])
			return handle("unsupported_curve");
		...
		//we add back return carriages
			pem_key =	pem_key.slice(0, 26) + "\n" + pem_key.substr(26);
			pem_key = pem_key.slice(0, 90) + "\n" + pem_key.substr(90);
			pem_key = pem_key.slice(0, -24) + "\n" + pem_key.substr(-24);
