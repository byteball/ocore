...
const BetterSQLController = require('./BetterSQLController');
...
module.exports = function(db_name, MAX_CONNECTIONS){
	...
	function openDb(cb){
		...
		return new BetterSQLController(path + db_name, cb);
	}
	...
	function connect(handleConnection){
		...
		//	if (!bCordova)
		//		db.serialize();
			connection.query("PRAGMA foreign_keys = ON", function(){
				console.log('next');
	...
	function takeConnectionFromPool(handleConnection){
		...
		console.log(arrConnections.length, MAX_CONNECTIONS);
