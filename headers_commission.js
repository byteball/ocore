					...
					rows => {
						...
						conn.query(
							...
							profit_distribution_rows => {
								const arrValues = [];
								profit_distribution_rows.forEach(({unit, earned_headers_commission_share, address}) => {
									const child_unit = unit;
									for (const payer_unit in assocWonAmounts[child_unit]){
										...
										const amount = (earned_headers_commission_share === 100) 
											? full_amount 
											: Math.round(full_amount * earned_headers_commission_share / 100.0);
										// hc outputs will be indexed by mci of _payer_ unit
										arrValues.push(`('${payer_unit}', '${address}', ${amount})`);
									}
								});
...
function getWinnerInfo(arrChildren){
	...
	arrChildren.sort(({hash}, {hash}) => (hash < hash) ? -1 : 1);
	return arrChildren[0];
}
